...
local HOME = os.getenv("HOME")local WIDGET_DIR = HOME .. '/.config/awesome/awesome-wm-widgets/battery-widget'
...
local function worker(user_args)
    ...
    local function show_battery_status(batteryType)
        awful.spawn.easy_async([[bash -c 'cat /sys/class/power_supply/BAT0/status /sys/class/power_supply/BAT0/capacity']],
        function(stdout, _, _)
...
--         end
	...
	if (charge > 80) then batteryType = "battery-full%s-symbolic"
	elseif (charge > 60) then batteryType = "battery-good%s-symbolic"
	elseif (charge > 40) then batteryType = "battery-low%s-symbolic"
	elseif (charge > 20) then batteryType = "battery-caution%s-symbolic"
	else
            batteryType = "battery-empty%s-symbolic"
